...
contract BLS {
    ...
    function mapToPointWithHelp(uint256[2] memory _x, uint256[2] memory expected_roots)
        public
        view
        returns (uint256[2] memory px, uint256[2] memory py)
    {
        ...
        y0 = addmod(y0, 19485874751759354771024239261021720505790618469301721065564631296452457478373, N);
        y1 = addmod(y1, 266929791119991161246907387137283842545076965332900288569378510910307636690, N);
        m0 = addmod(mulmod(expected_roots[0],expected_roots[0], N), mulmod(N-expected_roots[1],expected_roots[1], N), N);
	...
	function addPoints(uint256[2] memory p1, uint256[2] memory p2) internal returns (uint256[2] memory r) {
		uint[4] memory input;
		input[0] = p1[0];
		input[1] = p1[1];
		input[2] = p2[0];
	...
	function mulPoint(uint256[2] memory p, uint256 s) internal returns (uint256[2] memory r) {
		uint[3] memory input;
		input[0] = p[0];
		input[1] = p[1];
		input[2] = s;
    ...
    function verifyHelpedAggregated(uint256[2][2] memory points_g1, uint256[4] memory aggregated_g2, bytes memory data, uint256[2] memory signature) public {
        uint256[2] memory hash = hashToPoint(data);
        require(verifySingle(signature, aggregated_g2, hash), "verification failed");
        uint256[2] memory aggregated_p1 = addPoints(points_g1[0], points_g1[1]);
        emit Debug(gasleft(), 0, 0, 0);
        uint256 alpha = uint256(keccak256(abi.encodePacked(data, signature[0], signature[1], aggregated_g2[0], aggregated_g2[1], aggregated_g2[2], aggregated_g2[3], aggregated_p1[0], aggregated_p1[1])));
        emit Debug(gasleft(), 0, 0, 0);
        uint256[2] memory scaled_p1 = mulPoint(aggregated_p1, alpha);
        uint256[2] memory scaled_g1_generator = mulPoint([7875458235035678754887153468411793526875066621955642619646139314277366414792, 8106623690154677659962327366078301943507317780154727084061147098569974335996], alpha);
        ...
        emit Debug(gasleft(), 0, 0, 0);
        require(verifySingle(g1_part_sig, aggregated_g2, g1_part_hash), "verification failed");
        emit Debug(gasleft(), 0, 0, 0);
